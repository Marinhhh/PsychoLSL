 ...
 * \brief  使用 NatNet 库的示例客户端
 * 此程序连接到 NatNet 服务器，接收数据流，并将该数据流写入
 * ascii 文件。其目的是说明如何使用 NatNetClient 类。
 * 用法 [可选]:
 *	SampleClient [服务器IP] [本地IP] [输出文件名]
 *	[服务器IP]			服务器的 IP 地址（例如 192.168.0.107）（默认为本地机器）
 *	[输出文件名]	要写出的点文件 (pts) 的名称。默认为 Client-output.pts
 *********************************************************************/
