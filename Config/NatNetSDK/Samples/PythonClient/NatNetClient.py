...
#
# 本软件受 OPTITRACK 插件 EULA（最终用户许可协议）约束，可在
# https://www.optitrack.com/about/legal/eula.html 查看，和/或可与
# 相关软件文件一起下载（"插件 EULA"）。通过下载、安装、激活
# 和/或以其他方式使用软件，您同意您已阅读并同意遵守
# 插件 EULA 和所有适用的法律法规。如果您不同意受插件 EULA 约束，
# 则您不得下载、安装、激活或以其他方式使用软件，并且您必须立即删除或
# 退回软件。如果您代表一个实体下载、安装、激活和/或以其他方式使用
# 软件，那么这样做即表示并保证您有适当的权限代表该实体接受插件 EULA。
# 请参阅根目录中的许可文件以了解其他管理条款和信息。
# =============================================================================

# OptiTrack NatNet 直接解包 Python 3.x 库
...
class NatNetClient:
    # print_level = 0 关闭
    # print_level = 1 开启
    # print_level = >1 开启 / 每第n个动作捕捉帧打印一次
    print_level = 20

    def __init__(self):
        # 将此值更改为 NatNet 服务器的 IP 地址。
        self.server_ip_address = "127.0.0.1"

        # 将此值更改为您本地网络接口的 IP 地址
        self.local_ip_address = "127.0.0.1"

        # 应与 Motive 流设置中列出的组播地址匹配。
...
# Unpack Mocap Data Functions
    ...
    def __process_message(self, data: bytes, print_level=0):
        ...
        if message_id == self.NAT_FRAMEOFDATA:
            ...
            offset += offset_tmp
            # print("MoCap Frame: %d\n" % (mocap_data.prefix_data.frame_number))  # 注释掉以避免刷屏
